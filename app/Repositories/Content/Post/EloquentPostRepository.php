...
class EloquentPostRepository implements ContentRepository
{
    ...
    public function getMainByUserIds($userId)
    {
        ...
        if (Auth::check()) {
            $posts= Post::whereIn('users_id', $userIds)->where('news', '0')->orderby('id', 'desc')->where('ban','0')->orderby('id','desc')->paginate(4);
            $combined = $posts->merge(Post::where('news', '1')->where('state', '1')->orderby('id','desc')->paginate(2));
            $combined->all();
            return $combined;
        } else {
            return Post::where('news', '1')->orderby('id','desc')->paginate(4);
        }
...
public
function getMoreComments($postId, $commentsId)
{
...
public
function updateByUserId($id, User $user)
{
...
public
function deleteByUserId($id, User $user)
{
...
public
function likeByUserId($id, User $user)
{
...
public
function commentByUserId(Request $request, $postId, $user)
{
...
public
function shareByUserId($id, User $user)
{
...
public
function banByUserId($id, User $user)
{
...
public
function reportByUserId($id, User $user)
{
...
public
function update(Post $post, $data)
{
