...
@if($files->count() == 0)
    ...
    @endif
            ...
            @foreach($files as $file)
                ...
                @foreach($tags as $tag)
                    ...
                    <div class="col-md-2 col-sm-6">
                        <div data-toggle="modal" data-target="#file-{{$tag->file->first()->id}}" class="square-services"
                             style="height: 170px">
                            <img src="<?= Url('/images/0055.jpg'); ?>" alt=""/>
                            <div class="square-infos">
                                @if(Auth::check())
                                    @can('delete', $tag->file->first())
                                        <span style="float: left;margin-right: 15px;margin-top: 7px;font-size: 18px; color:#5e5e55">
                                        <a onclick="return confirm('آیا از حذف این فایل مطمئن هستید؟');" href="<?= Url('home/file/delete/'.$tag->file->first()->id); ?>"><small><i class="fa fa-remove"></i></small></a></span>
                                    @endcan
                                @endif


                                <i style="font-size: 20px" class="fa fa-file"></i><br>
                                <p>{!! \Illuminate\Support\Str::words($tag->file->first()->title , $words = 8, $end = '...') !!}</p>
                                @if(Auth::check())
                                    <a   href="<?= Url('/files/' . $tag->file->first()->link); ?>" title=""><i
                                                class="fa fa-cloud-download"></i></a>
                                @endif
                            </div>
                        </div><!-- square services -->
    ...
    @foreach($tags as $tag)
        <div class="modal fade" id="file-{{$tag->file->first()->id}}" tabindex="-1" role="dialog"
             aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        ...
                        <h3>{{$tag->file->first()->title}}</h3>
                        <br>
                        <p>{!! $tag->file->first()->description !!}</p>
                        <hr>
                        <span> توسط:  <a title="{{$tag->file->first()->user->name.' '.$tag->file->first()->user->family}}"   href="<?= Url('home/profile/show/137-'.$tag->file->first()->user->id.'-42-'.$tag->file->first()->user->username); ?>">{!! $tag->file->first()->user->username !!}</a> </span>
                        <hr>
                        <span> برچسب ها:
                            @foreach($tag->file->first()->tag as $tag)
                                {{$tag->title.', '}}
                            @endforeach
                        </span>
                        ...
                        <div class="modal-footer">
                            @if(Auth::check())
                                <a   href="<?= Url('/files/' . $tag->file->first()->link); ?>" type="button" name="_token"
                                   class="btn btn-success"><i class="fa fa-cloud-download"></i>
                                </a>
                            @endif
                            <button type="button" class="btn btn-default"
